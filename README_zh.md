...
通过 JSON-RPC 暴露 14 个独立工具（`officecli_view`、`officecli_get`、`officecli_set`、`officecli_add`、`officecli_remove`、`officecli_move`、`officecli_batch`、`officecli_schema` 等），每个工具有精确的 JSON Schema — 无需 shell 访问。
...
- **确定性 JSON 输出** -- 每个命令都支持 `--json`，返回统一 camelCase 格式的结构化数据。无需正则解析。
- **变更回显** -- 写操作（`set`、`add`、`move`）在 `--json` 模式下自动返回完整的更新后节点状态，省去额外的 `get` 调用。
- **错误教学** -- 拼错属性名时返回 `correctedArgs`，智能体可直接重试。例如：`colr` → `{"correctedArgs": {"props": ["color=FF0000"]}}`。
- **Schema 自发现** -- `officecli schema pptx shape` 以 JSON 格式返回所有可设置的属性及其类型、格式、示例值。智能体调用前就知道参数怎么写。
...
# 机器可读 Schema（为 AI 智能体设计）
officecli schema pptx           # 所有元素和常用属性
officecli schema pptx shape     # 详细属性定义（类型、格式、示例值）
officecli schema xlsx cell      # Excel 单元格属性
```
...
**写操作**（`set --json`、`add --json`、`move --json`）— 包含更新后的节点：
...
**属性自发现** — `officecli schema <format> [element]`：
...
错误码：`not_found`、`invalid_value`、`unsupported_property`、`invalid_path`、`unsupported_type`、`missing_property`、`file_not_found`、`file_locked`、`invalid_selector`。属性名支持自动纠错 -- 拼错属性名时会返回 `correctedArgs`，智能体可直接用修正后的参数重试。
...
**变更确认**（`set`、`add`、`move` 使用 `--json`）包含更新后的节点：
...
| `check` | 扫描布局问题（文本溢出、重叠等） |
| `schema` | 以 JSON 输出属性定义（AI 智能体自发现） |
