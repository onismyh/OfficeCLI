...
static partial class CommandBuilder
{
    ...
    internal static int SafeRun(Func<int> action, bool json = false)
    {
        if (!OfficeCli.Core.CliLogger.Enabled)
        {
            ...
            {
                return RetryOnFileLock(action, ex, json);
            }
            catch (Exception ex)
            {
                WriteError(ex, json);
    ...
    {
        // HResult 0x80070020 = ERROR_SHARING_VIOLATION (Windows)
        // HResult 0x80070021 = ERROR_LOCK_VIOLATION (Windows)
        var hr = ex.HResult & 0xFFFF;
        if (hr is 0x0020 or 0x0021) return true;
        // On Unix/macOS, check the message for common lock-related patterns
        var msg = ex.Message;
        return msg.Contains("being used by another process", StringComparison.OrdinalIgnoreCase)
            || msg.Contains("locked", StringComparison.OrdinalIgnoreCase)
            || msg.Contains("sharing violation", StringComparison.OrdinalIgnoreCase);
    }

    private static int RetryOnFileLock(Func<int> action, IOException originalEx, bool json)
    {
        const int maxRetries = 3;
        for (int attempt = 1; attempt <= maxRetries; attempt++)
        {
            Thread.Sleep(attempt * 500); // 500ms, 1s, 1.5s
            try
            {
                return action();
            }
            catch (IOException ex) when (IsFileLockException(ex))
            {
                ...
                {
                    WriteError(new CliException(
                        $"File is locked by another process after {maxRetries} retries: {originalEx.Message}")
                    {
                        Code = "file_locked",
                        Suggestion = "Close the file in other applications and try again."
                    }, json);
                    return 1;
                }
            }
    ...
    internal static void PrintBatchResults(List<BatchResult> results, bool json)
    {
        if (json)
        {
            var batchJson = System.Text.Json.JsonSerializer.Serialize(results, BatchJsonContext.Default.ListBatchResult);
            ...
            {
                ["success"] = allSuccess,
                ["data"] = System.Text.Json.Nodes.JsonNode.Parse(batchJson),
    ...
    private static void NotifyWatch(IDocumentHandler handler, string filePath, string? changedPath)
    {
        if (handler is OfficeCli.Handlers.ExcelHandler excel)
        {
            ...
            WatchNotifier.NotifyIfWatching(filePath, new WatchMessage { Action = "full", FullHtml = handler.ViewAsHtml(), ScrollTo = scrollTo });
            return;
        }
        ...
        if (slideNum > 0)
        {
            ...
            if (html != null)
            {
                WatchNotifier.NotifyIfWatching(filePath, new WatchMessage { Action = "replace", Slide = slideNum, Html = html, FullHtml = handler.ViewAsHtml() });
                return;
            }
        }
        WatchNotifier.NotifyIfWatching(filePath, new WatchMessage { Action = "full", FullHtml = handler.ViewAsHtml() });
    }
    ...
    private static void NotifyWatchRoot(IDocumentHandler handler, string filePath, int oldSlideCount)
    {
        ...
        if (newCount > oldSlideCount)
        {
            ...
            if (slideHtml != null)
            {
                WatchNotifier.NotifyIfWatching(filePath, new WatchMessage { Action = "add", Slide = newCount, Html = slideHtml, FullHtml = handler.ViewAsHtml() });
                return;
            }
        }
        else if (newCount < oldSlideCount)
        {
            WatchNotifier.NotifyIfWatching(filePath, new WatchMessage { Action = "remove", Slide = oldSlideCount, FullHtml = handler.ViewAsHtml() });
            return;
        }
        WatchNotifier.NotifyIfWatching(filePath, new WatchMessage { Action = "full", FullHtml = handler.ViewAsHtml() });
    }
