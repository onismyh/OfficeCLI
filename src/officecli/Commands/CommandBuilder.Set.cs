...
{
    private static Command CreateSetCommand(Option<bool> jsonOption, Option<bool> forceOption)
    {
        ...
        setCommand.SetAction(result => { var json = result.GetValue(jsonOption); return SafeRun(() =>
        {
            ...
            if (json)
            {
                ...
                if (allFailed)
                {
                    // Check if we can build correctedArgs for the error
                    var correctedProps = new List<string>();
                    bool hasCorrectable = false;
                    foreach (var u in stillUnsupported)
                    {
                        var rawKey = u.Contains(' ') ? u[..u.IndexOf(' ')] : u;
                        if (properties.TryGetValue(rawKey, out var val))
                        {
                            var suggestion = SuggestProperty(rawKey);
                            if (suggestion != null)
                            {
                                correctedProps.Add($"{suggestion}={val}");
                                hasCorrectable = true;
                            }
                        }
                    }
                    ...
                    if (hasCorrectable)
                    {
                        throw new OfficeCli.Core.CliException(
                            $"UNSUPPORTED props: {string.Join(", ", stillUnsupported)}")
                        {
                            Code = "unsupported_property",
                            Suggestion = string.Join(", ", stillUnsupported.Select(u =>
                            {
                                var rawKey = u.Contains(' ') ? u[..u.IndexOf(' ')] : u;
                                return SuggestProperty(rawKey);
                            }).Where(s => s != null)),
                            Help = "officecli help <format>-set",
                            CorrectedArgs = new Dictionary<string, object>
                            {
                                ["command"] = "set",
                                ["path"] = path,
                                ["props"] = correctedProps.ToArray()
                            }
                        };
                    }
                ...
                {
                    // Success path: echo updated node when properties were applied
                    DocumentNode? echoNode = applied.Count > 0 ? handler.Get(path) : null;
                    Console.WriteLine(OutputFormatter.WrapEnvelopeWithNode(outputMsg, echoNode, allWarnings.Count > 0 ? allWarnings : null));
                }
            }
            else
            {
