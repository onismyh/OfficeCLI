...
{
    private static Command CreateValidateCommand(Option<bool> jsonOption)
    {
    ...
    private static Command CreateCheckCommand(Option<bool> jsonOption)
    {
        ...
        checkCommand.SetAction(result => { var json = result.GetValue(jsonOption); return SafeRun(() =>
        {
            ...
            if (json)
            {
                ...
                Console.WriteLine(OutputFormatter.WrapEnvelope(
                    dataObj.ToJsonString(OutputFormatter.PublicJsonOptions)));
            }
            else
            {
